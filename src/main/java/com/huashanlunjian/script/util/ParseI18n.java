...
public final class ParseI18n {
    ...
    public static ITextComponent parse(String strIn) {
        ...
        if (strIn.startsWith(I18N_START_CHAR) && strIn.endsWith(I18N_END_CHAR)) {
            // 将剔除大括号后的字符进行国际化
            return ITextComponent.getTextComponentOrEmpty(strIn.substring(1, strIn.length() - 1));
        } else {
            // 否则装填原字符串（这算硬编码么？）
            return ITextComponent.getTextComponentOrEmpty(strIn);
        }
    }
     ...
     * 将 key 列表转换成对应的翻译文本
     */
    //public static List<ITextComponent> keysToTrans(List<String> keys, ChatFormat... formatting) {
    //    List<ITextComponent> out = Lists.newArrayList();
    //    for (String k : keys) {
    //        out.add(ITextComponent.translatable(k).withStyle(formatting));
    //    }
    //    return out;
    //}
}
